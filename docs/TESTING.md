...
| Unsigned IPA reproducibility | Pass — two timestamp-normalized RTC/lifecycle release-candidate packages were byte-identical with SHA-256 `a9e85a86a9cc2956ab0d161080b418bb9d358b692551e9cba21d9fb167914075`; the preceding post-notice checkpoint also reproduced from an independent fresh clone |
...
| Animal Crossing through Aurora/Metal | Partial — native ARM64 executable selects Metal, loads disc/assets/audio, and reaches an interactive title menu at 60 FPS; EFB clearing prevents accumulation, palette/cache fixes hold, counter-clockwise WebGPU front faces restore complete title geometry, keyboard A advances correctly rendered multi-line K.K. dialogue, and iPhone renders animated title water/choice UI plus a saved-player train/station sequence. Interiors, broader outdoor traversal, and long-session memory still need direct evidence |
