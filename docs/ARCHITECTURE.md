...
GCI-folder mode is the initial canonical store because it gives one file per save and aligns with Dolphin import/export. Writes go to a temporary sibling, are flushed and synchronized, then atomically replace the canonical file after three previous generations rotate; the parent directory is synchronized on Apple/POSIX hosts. Before the mobile core starts, Bellpad strictly checks exact size, game/version/block metadata, town identity, and the town checksum. An invalid canonical file is atomically replaced from the newest valid `.bak1`–`.bak3` or pre-import backup, and Foundation is explicitly told to retain the displaced damaged bytes under a unique timestamped `.gci.corrupt-…` quarantine name. That name deliberately does not end in `.gci`, so the legacy directory scan cannot rediscover it. With no valid backup, Bellpad moves the damaged canonical file to quarantine and lets the core start without it; if either preservation operation fails, startup stops instead of exposing invalid data to the permissive legacy loader. A native notice explains every recovery outcome. Live iPhone checksum-corruption fault injection proves both exact-byte restoration plus damaged-byte retention and the no-backup quarantine path.

The same Bell/Cove GCI has completed desktop creation/relaunch, iPhone rewrite/relaunch, an isolated Dolphin 5.0-17995 GCI-folder read, and Bellpad pre-boot installation/loading of the Dolphin-managed copy. Mobile export creates a validated immutable snapshot for Files; real iPhone/iPad exports matched the canonical bytes. Import accepts only exact-size GAFE01 version 5/6 GCI data with the expected block count, town-ID mask, and zero-sum town checksum; it durably stages the file, installs it before the next game startup, and retains the previous canonical file as a pre-import backup so live game state is never swapped underneath the core. A fully UI-driven iPhone Files import selected, staged, and installed the canonical byte-identical GCI before normal startup. A versioned in-place Simulator update preserved the exact save and retained-image inodes/hashes plus `NSUserDefaults`, and the replacement bundle loaded that state normally. The retail in-game save dialogue remains a validation gate.
...
- Touch targets a portable, mutex-protected normalized GameCube state that the linked game-core adapter snapshots on the game thread before Aurora PAD/event processing. Physical controllers remain owned by Aurora's SDL3 PAD layer so its instance identities, saved mappings, and player slots stay authoritative; the UIKit GameController observer is used only to hide or restore the touch overlay.
- Aurora validates stored SDL3 handles before gameplay reads and reconciles them against current enumeration at startup, add/remove/remap events, foreground resume, and a bounded active check. It closes stale ownership before opening returns, preserves connected slots, assigns a sole return to player 1 and additions to the next free slot, and never restarts the controller subsystem. Touch and any obsolete native controller state are cleared on resign/activation boundaries.
