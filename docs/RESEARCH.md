...
| Full 64-bit migration | https://github.com/birabittoh/ACGC-PC-Port — `master` `915fb86ba9a6c2144dabda9143d93af7a3f92be7` (upstream PR #48) | Inherits CC0/MIT tree; provenance derives from the PC port and PR history | Built locally as macOS ARM64. Bellpad's six tracked patches repair aligned DVD tails, input edges, Apple Clang/native-width portability, the structure-actor pool overflow, expose SDL-independent native text commits, and add normalized virtual-pad state. Both Apple Clang 21 and GCC 16 complete the build; the Clang runtime reaches the rendered title and repeatedly tears down/reloads it without the former invalid free. Town entry is proven, while save/reload and lifecycle teardown remain open | Milestone 1 fault-isolation baseline and migration evidence; changes must be reviewed/forward-ported |
