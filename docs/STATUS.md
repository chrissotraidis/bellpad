...
- Tracked scripts now reproduce the pinned checkout, apply all fourteen clean core compatibility patches and both Aurora compatibility patches idempotently, build an Apple Clang ARM64 Mach-O or playable `Bellpad.app`, compile/audit the Aurora-defined full core, allow an isolated GCC cross-check, and optionally link or select (never copy) a local image.
...
- No Animal Crossing-on-Aurora-GX render has been demonstrated locally. Symbol coverage, host-object ABI, complete-core compilation, and legacy-renderer isolation now pass, but the real executable/platform link and representative `emu64`/scene rendering remain.
