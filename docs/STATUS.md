...
- Tracked scripts now reproduce the pinned checkout, apply all thirteen clean core compatibility patches and the Aurora compatibility patch idempotently, build an Apple Clang ARM64 Mach-O or playable `Bellpad.app`, allow an isolated GCC cross-check, and optionally link or select (never copy) a local image.
...
- No Animal Crossing-on-Aurora-GX render has been demonstrated locally. Symbol coverage and host-object ABI now pass, but the real executable/platform link and representative `emu64`/scene rendering remain.
...
| 11–21 | Rendering/platform behavior | Desktop game rendering/audio/input/text partial; mobile Metal shell, safe-area touch, controller merge, and resign-active clearing pass; game rendering/audio/saves/RTC/memory pending |
