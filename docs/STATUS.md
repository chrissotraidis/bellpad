...
- The first title image through Aurora is recognizable, including menu text and choices, but colors, textures, and portions of the scene are visibly corrupted. This is command-flow and integration evidence only; correct rendering is not claimed.
...
- Tracked scripts now reproduce the pinned checkout, apply all sixteen clean core compatibility patches and both Aurora compatibility patches idempotently, build an Apple Clang ARM64 Mach-O, playable `Bellpad.app`, or Aurora/Metal convergence executable, compile/audit the Aurora-defined full core, allow an isolated GCC cross-check, and optionally select (never copy) a local image.
...
- Animal Crossing now renders through Aurora GX/Metal and per-frame clearing is correct, but the emu64-rendered logo pieces still have incorrect placement and color while host-native menu text is stable. Matrix/projection, palette/texture, and retained GX state are the immediate convergence audit.
...
| 11–21 | Rendering/platform behavior | OpenGL desktop game path is correct; Aurora/Metal title is corrupted but audio/frame pacing run; mobile shell safe-area touch, controller merge, and resign-active clearing pass; saves/RTC/lifecycle/memory remain |
