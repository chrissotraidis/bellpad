...
4. [x] Feed the title scene's Animal Crossing display lists through Aurora GX. Explicit host EFB clearing removed the accumulated-frame smear; static host-endian palette conversion now matches the dynamic path, cache invalidation is bounded per frame, and unsafe draw merging is disabled. Missing/incorrectly placed emu64 geometry remains, so matrix, projection, vertex interpretation, and retained state are the active gates.
