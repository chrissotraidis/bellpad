...
- Audio: [x] native SDL3/CoreAudio route with ambient/mixing AVAudioSession ownership at the requested 32 kHz rate, UIKit interruption/route/media-service latching, and game-thread pause/resume only after session readiness. Spaced synthetic interruption and route events pass on both simulator classes. Real calls, Bluetooth/headset routes, suspension queue behavior, and long hardware sessions remain device gates.
