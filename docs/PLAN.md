...
The Apple wrappers are converged: macOS, iPhone, and iPad builds exist; normalized touch/controller input, the gear menu, persistent per-device layouts, Native/1×/2×/3×/4× rendering, Files import, native setup text, GCI persistence, native save/disc management, deterministic audio-session recovery, saved-player train/station rendering on both simulator classes, one iPhone outdoor-town entry, and isolated Dolphin GCI-folder interchange have runtime evidence. Do not spend ongoing implementation time replaying every button across every activity. Finish the product through broader scene/lifecycle hardening, compressed-disc/editor/NES gaps, and physical-device validation. The exhaustive activity matrix remains a final acceptance pass.
...
4. [x] Run from trademark/title through town creation, save, process exit, relaunch, and persistence. An isolated Bell/Cove run reached its generated town, invoked the real game-facing save routine from initialized state, wrote a canonical GCI, closed normally, and reloaded the saved-player flow in a new process. iPhone Simulator loaded the same GCI through the shared native core, atomically rewrote it with `.bak1` rotation, and loaded the rewrite after app relaunch. Dolphin 5.0-17995 loaded that canonical file from an isolated GCI Folder, and Bellpad installed and loaded the Dolphin-managed copy before startup. Real Files exports on iPhone and iPad matched it byte-for-byte, and a fully UI-driven iPhone Files import staged and installed the same bytes on normal relaunch. The retail save-dialogue interaction remains a separate product gate.
5. Record rendering, audio, input, RTC, memory, and sanitizer evidence. Rendering, 32 kHz stereo audio, keyboard buttons, analog movement, desktop text input, host-local RTC initialization, subsecond conversion, activation rebasing, and a bounded ASan/UBSan title smoke have evidence; Resetti/time-travel behavior, long-session memory, and broader sanitized gameplay remain.
...
7. [x] Remove compiler-blocking GCC-only pointer truncation behavior so Apple Clang can compile and run the core. The first ASan/UBSan title pass repaired additional heap, message, sentinel, array, and audio-state boundaries; continue the broader address inventory because one bounded scene is not proof that every guest/native boundary is correct.
...
- Saves: [x] Application Support GCI storage, checksums, durable atomic replacement, rolling backups, strict pre-boot validation, exact-byte backup restoration, damaged-file quarantine, native recovery notices, desktop creation/relaunch, iPhone rewrite/relaunch, native import/export document pickers, isolated Dolphin GCI-folder interchange, real iPhone/iPad Files exports, a fully UI-driven iPhone Files import with byte-identical pre-boot installation, and versioned in-place app-update persistence for saves, retained game data, and settings. Imports are validated and staged for pre-boot installation with a pre-import backup; exports use immutable validated snapshots. Exercise the retail save dialogue. Consider raw-card support after GCI is stable.
...
- Audio: [x] native SDL3/CoreAudio route with ambient/mixing AVAudioSession ownership at the requested 32 kHz rate, UIKit interruption/route/media-service latching, and game-thread pause/resume only after session readiness. During a stopped output device, the producer advances at the native DAC sample clock while discarding samples, preventing command-queue saturation; resume clears stale stream/ring data. Eight-second interruptions and route events pass on both simulator classes. Real calls, Bluetooth/headset routes, and long hardware sessions remain device gates.
